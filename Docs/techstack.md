...
- `/org/*` calling modes: every `/org/*` route authenticates the domain pairing (per-domain hash bearer + verified signed config JWT bound to `?domain=`). With an `X-UOA-Access-Token` the acting user's org/team roles gate the call as before; with the header omitted the pairing alone authorises it and there is no acting user, so per-user role checks do not apply. Backend mode is opt-in per domain via `org_features.backend_org_management` (default false) and binds to the verified config domain, never the raw query value. Backend-initiated mutations are audited with `actorUserId: null` plus `metadata.uoa_actor = { via: "domain_backend", source_domain }`.
- Confidential delegation policy: `confidential_delegation_mappings` binds an authenticated `client_domains` row + product to one exact HTTPS resource and an explicit `ai.invoke` / `billing.read` / `token.provision` allowlist. Every application-to-application hop authenticates with the immediate caller application's own credential and mapping. Chained UOA access tokens require the exact caller audience, narrow through the inbound token and both source/caller mappings, cannot outlive the inbound token, and preserve provenance in the `act` chain. Token provisioning is never implied by AI invocation. It is admin-only under forced RLS; there is no process-env fallback. `token.provision` authorises provisioning against the mapping's own exact HTTPS resource only; it does not reach UOA's own `/org/*` surface, which authenticates the domain pairing directly. Signing these tokens requires `MCP_OAUTH_ACCESS_TOKEN_PRIVATE_JWK`.
