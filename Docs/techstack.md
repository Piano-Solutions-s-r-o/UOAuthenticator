...
- Confidential delegation policy: `confidential_delegation_mappings` binds an authenticated `client_domains` row + product to one exact HTTPS resource and an explicit `ai.invoke` / `billing.read` / `token.provision` allowlist. Every application-to-application hop authenticates with the immediate caller application's own credential and mapping. Chained UOA access tokens require the exact caller audience, narrow through the inbound token and both source/caller mappings, cannot outlive the inbound token, and preserve provenance in the `act` chain. Token provisioning is never implied by AI invocation. It is admin-only under forced RLS; there is no process-env fallback. A `token.provision` mapping whose resource is exactly `<PUBLIC_BASE_URL>/org` additionally lets the source domain drive UOA's own `/org/*` organisation and team routes server-to-server, bounded by the acting user's live roles; the resulting audit rows carry the calling product's provenance under the `uoa_actor` metadata key. Both the signing and the resource-side verification of these tokens require `MCP_OAUTH_ACCESS_TOKEN_PRIVATE_JWK`.
