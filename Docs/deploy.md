...
5. Exercise credentialed current-month `group_by=service` and `group_by=user`
   ...
   distinct current cumulative cursors for the same exact team, force database
   arrival order to differ from Ledger capture order, and prove every read
   returns the newest projection, older captures are successful superseded
   no-ops, usage is debited only once, and every persisted snapshot is complete.
   Replay the winning cursor and prove it adds no adjustment or debit. Then
