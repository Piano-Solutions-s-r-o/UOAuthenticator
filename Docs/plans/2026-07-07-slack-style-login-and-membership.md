...
| **Workspace / team**                                           | **`Team`**                                            | The unit users join, get invited to, and select at login. See §9.1 for why _not_ `Organisation`.                                              |
| Workspace membership                                           | `TeamMember` (+ `OrgMember`)                          | Exists, but has **no lifecycle status** today — the biggest gap.                                                                              |
| Channel / channel membership                                   | _Out of scope_                                        | UOA stores no channels; conversation-level ACL is the consuming app's concern (consistent with "consumer-defined roles" in roles-and-acl.md). |
...
The core Slack lesson — _never put permissions on the user; put them on memberships scoped to org/workspace_ — is already UOA's architecture. What UOA is missing is the **lifecycle** (invited → active → deactivated → removed), the **email-first login with workspace selection**, and a **first-class auth-identity record**.
...
- **Sessions**: `RefreshToken` bound to `{ userId, domain, clientId, configUrl, orgId?, teamId? }` with family rotation + reuse detection; `User.tokenVersion` gives **global** (all-domains) revocation only. Scoped membership lifecycle changes revoke exact org/team families across issuing product domains, while org lifecycle also revokes legacy same-domain unscoped sessions.
- **Identity**: account unification is merge-by-email; no record of _which_ providers a user has linked (the `method` token claim in `api-changes-rebac.md §5` has nowhere to be persisted).
...
> **Shipped correction:** the uniqueness constraint is `@@unique([userId, provider])` (one identity row per provider per user — the upsert target), with a non-unique `@@index([provider, providerSubject])` for social-subject lookups. The design above originally specified a _global_ `@@unique([provider, providerSubject])`, which was a design bug: under `user_scope: "per_domain"` the same email or social subject legitimately maps to separate `User` rows on different domains, and a global unique constraint would reject the second one and break the backfill. Verified against `API/prisma/schema.prisma` and migration `20260707104937_slack_membership_foundation`.

- Written on every successful login (upsert): social callbacks record the provider subject; email-link/code logins record an `email` identity. Existing behaviour (merge-by-verified-email, avatar overwrite per brief §22.7) is unchanged — this table _records_ the merge instead of losing it.
...
- This gives the `method` claim (`api-changes-rebac.md §5`) a persistence home _in principle_, gives SCIM's `scimExternalId` a natural future sibling, and lets per-provider auto-enrolment (`VerificationMethod` matching) audit _which_ identity satisfied the rule. **Shipped status:** `AuthIdentity` persistence itself shipped exactly as designed; the `method` access-token claim was not wired up to read from it — see §5 and §12b.

### 4.3 Email-first login: sign-in codes + workspace selection (G3, G4)

Slack's flow, translated into UOA's popup/config architecture. Everything happens **inside the Auth window for one verified client**. Legacy clients list teams on that config domain. A UOA-recognized product client may list every exact ACTIVE user org/team through the central app-key policy described in §6/§8; this is an explicit server-owned product capability, not a general cross-domain config option.
...
3. `POST /auth/select-team` — body `{ login_token, teamId }` (or `{ login_token, inviteId }` to accept a pending invite, or `{ login_token }` alone when the chooser is skipped/empty). Checks, in order — Slack's step 5:
   ...
   - team/org auth policy: resolve 2FA via existing `resolveTwoFaPolicy` for the _selected_ org — may return a `twofa_token` challenge instead of completing;
...
4. Password login (`POST /auth/login`) and social callbacks route into the same selection step when `workspace_selection` is on: instead of finalizing immediately, they return the same `{ login_token, teams, pending_invites }` payload. With `workspace_selection: "off"` (default), legacy clients remain unchanged. A server-recognized product suppresses the chooser but must resolve one exact ACTIVE workspace (or required first placement) before 2FA; ambiguous choices fail closed.
...
**Magic links join the same flow**: the email-link landing (`/auth/email-*` routes) currently finalizes directly; when `workspace_selection` is on, a consumed link also lands on the chooser (the link _is_ the verification). This keeps one canonical post-verification path: `verified identity → chooser → select-team → policy → code`.
...
- `AuthorizationCode` + `RefreshToken`: add nullable `orgId` / `teamId` columns. Populated by explicit/auto selection and by recognized-product pre-binding even when the chooser is off; only legacy unresolved flows remain null. `AuthorizationCode.twoFaCompleted` records interactive proof for current-policy revalidation at exchange.
...
- Refresh re-resolution validates the exact scoped membership and any product mapping. If the `active` org/team is no longer `ACTIVE`, or a cross-product mapping is revoked/expired/ambiguous, refresh fails closed without consuming or rotating the token. The user must restart interactive selection. It never drops `active` and silently switches or creates a tenant. Legacy same-domain/off refresh tokens remain unscoped during ordinary rotation, but an org deactivation/removal explicitly revokes them under the historical same-domain contract. Deactivation enforcement therefore needs no token-version bump.
- Every refresh decision locks exact `(userId, domain)` and re-reads its row before deciding reuse or rotation. Scoped refresh then locks the exact organisation membership followed by the exact team membership. Org lifecycle takes the same session lock before those membership locks and commits its status plus scoped/legacy revocation atomically. A refresh-first race leaves its replacement revoked by the waiting writer; a lifecycle-first race is observed by the re-read and creates no replacement, including for legacy unscoped rows.
- Exact workspace scope is server-owned for recognized products and cannot be disabled by signed config flags. Every interactive identity path pre-binds that scope before 2FA, including `workspace_selection: "off"`; an originally unscoped auto flow may likewise bind one exact eligible ACTIVE workspace. Multiple choices fail closed. Zero choices create a personal workspace only when `user_needs_team` explicitly permits placement, serialized by a per-user transaction advisory lock; otherwise a recognized product fails closed. Legacy same-domain/off exchange remains unscoped when its placement is already satisfied. Refresh rotation never runs placement.
- `GET /auth/email/link` and `POST /auth/verify-email` keep workspace choices/resolution, exact policy/2FA finalization, and immediate authorization-code issuance in one BYPASSRLS admin transaction. This is the lifetime of the first-placement advisory lock, so simultaneous first use from distinct recognized products cannot both act on a zero-workspace read. Downstream issuance failure rolls back placement and code together; the independently committed one-time email capability remains consumed.
- The exact selected Organisation participates in strongest-wins 2FA even when it belongs to another product domain. Authorization-code exchange re-resolves the current policy and enrollment after exact membership validation and requires the persisted `twoFaCompleted` proof whenever policy now requires it; rejection rolls back code consumption and creates no token family.
- Auth-code, refresh, and confidential issuance hold a shared global product-policy advisory lock through commit; supported domain/service/lifecycle-key mutators hold its exclusive form. The authenticated ClientDomain is re-read under that lock. Confidential verification happens before the transaction, while final delegation, product-aware exact workspace validation, one-time assertion consumption, and signing happen inside it. Recognized product assertions must include `active`.
...
| Operation                                                                               | Effect                                                                                                                                                                                                                                                                                                                             |
| --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Deactivate org member** — `POST /org/organisations/:orgId/members/:userId/deactivate` | `OrgMember.status = DEACTIVATED`; all that org's `TeamMember` rows → `DEACTIVATED`; revoke live refresh-token families for exact `(userId, orgId)` across every issuing product domain plus same-domain legacy/unscoped sessions, in the same fail-closed transaction; audit row. `LoginLog`, invites, and audit history remain.   |
...
| **Remove from team** — existing `DELETE .../teams/:teamId/members/:userId`              | Becomes `TeamMember.status = REMOVED` (tombstone) instead of row delete. Org identity stays active; every live refresh family for exact `(userId, teamId)` is revoked across issuing product domains in the same transaction. Other-team sessions are untouched. Existing "cannot leave your last team" counts `ACTIVE` rows only. |
| **Remove from org** — existing `DELETE .../members/:userId`                             | `OrgMember` + that org's `TeamMember` rows → `REMOVED`; the org's `GroupMember` rows are deleted. Exact `(userId, orgId)` families across all product domains plus same-domain legacy/unscoped sessions are revoked atomically as for deactivation.                                                                                |
...
Reactivation and re-add change membership state only; they never clear refresh-token revocation,
so an old family cannot regain access after the user returns.
...
Landing at `/auth/team-invite-link/:token` requires the visitor to complete normal email verification (link or code) before membership is granted — the link authorizes _joining_, never _authentication_. `useCount` increments atomically on successful join; expiry/revocation/cap failures render the generic invalid-link page. Redemption locks the exact membership rows and refuses an existing inactive organisation tombstone, so it cannot silently reactivate that organisation membership or consume the link. Per Slack, invite links are refused (creation blocked) when the team's effective policy forbids self-serve entry (`HIDDEN`) — and later, when SSO-required orgs exist, links are disabled there too.
...
Written from every lifecycle/invite/policy mutation. **Shipped correction:** rows are written best-effort _after_ the mutation's own transaction has committed, via `auditOrg` (`organisation.service.base.ts`) calling `writeOrgAuditLog` (`org-audit-log.service.ts`) against the BYPASSRLS admin client, wrapped in a try/catch that swallows failures — not in the same transaction as the mutation. This is deliberate: an audit-write failure must never roll back (or appear to roll back) a mutation that has already succeeded. (`writeOrgAuditLog` does accept a tenant tx client as an option for a same-transaction write, but no current caller passes one.) Read endpoint (`GET /org/organisations/:orgId/audit-log`, owner/admin) was not built — deferred, see §12b. Finite retention like `LoginLog` (brief §22.8) also not yet implemented.

**Gap closed (team membership):** "written from every lifecycle mutation" was aspirational for team membership until now. `team_member.added` was only ever written by the two self-service paths (`selfJoinTeam`, `redeemTeamInviteLink`), and `team_member.removed` / `team_member.role_changed` were declared-but-dead enum members — the three manager-driven services (`addTeamMember`, `changeTeamMemberRole`, `removeTeamMember` in `team.service.members.ts`) wrote no row at all, so an owner/admin — or a product backend holding a confidential `token.provision` token (§5, PR #19) — could add, re-role, or remove a team member leaving no trace. All three now call `auditOrg` with `actor` provenance threaded from the route via `getActorProvenance`, and tag `metadata.via = "manager"` so the audit stream distinguishes a manager acting on somebody else's membership from `self_join` / `invite_link`. Covered by `API/tests/unit/team.service.members.audit.test.ts`.
...
```jsonc
{
  ...
  "org": {
    /* unchanged LEGACY flat shape — a single org context, as today */
  },
  "active": {
    // NEW, optional — the workspace this session was opened for
    "orgId": "org_abc",
    "teamId": "team_xyz",
  },
}
...
| Path                                                      | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /auth/login`, social callbacks, email-link landings | With `workspace_selection: "auto"`: return/land on chooser instead of finalizing directly. **Shipped and verified on all three paths** (originally the design's biggest open question): `login.ts` returns the chooser payload inline; social callbacks (`callback.ts`) redirect with a `login_token` + `flow=workspace_chooser` (hydrated via the new `/auth/session-choices`, above); and — landing last, in the gap-fix B round (commit `6a226ab`) — the magic-link paths: `GET /auth/email/link` (`email-registration-link.ts`) redirects the same `login_token`/`flow=workspace_chooser` way, and `POST /auth/verify-email` (`verify-email.ts`) returns the inline JSON chooser payload like `login.ts`. Invite-bound verification tokens (`teamInviteId` set) never see the chooser — the accepted invite _is_ the selection. A centrally bound product client (active `ClientDomain` plus one unambiguous active/current `CUSTOMER_LIFECYCLE` app-key service mapping whose `actorIssuer` is exactly the product's HTTPS config domain) intentionally sees every exact ACTIVE org/team membership, enabling first direct login without circular service-access evidence. Legacy, unknown, inactive, expired, revoked, and ambiguous product mappings stay same-domain; pending invites stay same-domain. Cross-product enumeration runs only through `uoa_admin`, never a tenant transaction. Both email routes keep that expansion, first placement, final policy decision, and immediate code issuance inside one admin transaction so the placement lock cannot expire between the zero-choice read and commit. |
| `POST /auth/token`                                        | Access token may carry `active` next to the legacy `org` claim (§5), and `firstLogin.memberships` is built from the same centrally resolved client/product policy so it includes the selected cross-domain org/team. Code issuance, code consumption, and signing all revalidate the exact scope. A scoped refresh fails closed (and rolls rotation back) when its mapping or either membership is no longer valid; it never silently drops `active`, switches tenant, or auto-creates a product-domain workspace. Same-domain sessions remain on the legacy policy even if a product key is revoked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- Cross-product choices are a server-owned UOA policy, not an RP claim. Eligibility requires one
  active config domain and exactly one active BillingService resolved from current, unrevoked,
  unexpired `CUSTOMER_LIFECYCLE` keys with the exact HTTPS issuer. Every matching key must resolve
  to that same service. The choice still requires exact ACTIVE org and team membership; any control
  plane ambiguity falls back to same-domain behavior.
...
Original design phasing described Phase 3 as a single unit; it shipped split into 3a/3b/3c, each independently committed. The config defaults preserve legacy-client behavior (`email_code_enabled: false`, `workspace_selection: "off"`, `joinPolicy: INVITE_ONLY`, `member_invites: "allowed"` being the only default-on behavioral addition), while recognized products always retain server-owned exact workspace attribution even when the chooser is off.
...
| **UOA Admin** (`/Admin`)               | UOA        | Operator views only (domains, integrations, platform superusers). Workspace admins are _product_ users and never see this panel. Operator-facing org/member browsing over `/internal/admin/*` may adopt the same status vocabulary later, but is not part of this design. |
...
active team and no pending invites, or when `workspace_selection: "off"`. For a recognized product,
"off" still pre-binds one exact workspace before policy evaluation. New/empty users with
...
4. **Chooser continuation/replay hardening (2026-07-19).** The original `login_token` signed only `{ sub, domain }`, allowing a valid bridge to be replayed or paired with another allowlisted redirect/PKCE/config continuation. It now signs every security-relevant continuation field plus a canonical parsed-config fingerprint and JTI. The final-selection transaction claims a unique admin-only `login_session_uses` row containing only the hashed JTI as its first statement, so a concurrent replay loses before invite, audit, code, or access-request email effects. Any later failure rolls the claim and all database effects back so the original continuation can be retried. Choices and decline remain non-consuming.
...
6. **Team deletion/membership linearization (2026-07-19).** Non-default team deletion locks the current organisation and exact target team rows inside its transaction, derives the target's existence/default status from that locked row, then enumerates members. PostgreSQL's foreign-key `KEY SHARE` lock gives concurrent brand-new membership insertion one serial result: insert-first commits before enumeration and the member is included/re-homed to the default team when necessary; delete-first blocks the insert until the parent is gone, then the insert fails. Existing affected users are locked in stable user order across all of their organisation team memberships, and deletion re-reads ACTIVE target rows only after those locks. Team-member removal takes the same org-wide membership lock and re-reads the exact ACTIVE target row before mutation, so a removal that commits first cannot be mistaken for an ACTIVE member and silently re-added to the default team.
7. **Cross-product refresh-family lifecycle linearization (2026-07-21).** A product-issued
   refresh family carries UOA's exact org/team IDs even when its config domain differs from that
