...
- \`redirect_urls\` MUST be a non-empty array of redirect targets. Each entry is one of: an \`https://\` URL (any host); an \`http://\` URL on loopback only (\`localhost\`, \`127.0.0.1\`, or \`[::1]\` — for a native app's transient loopback listener, RFC 8252); or a native custom-scheme deep link (e.g. \`com.acme.app://callback\`, \`nessie://auth/callback\`). Non-loopback \`http://\` and dangerous schemes (\`javascript:\`, \`data:\`, …) are rejected. The runtime \`redirect_url\` must match one of these entries **byte-for-byte**, including scheme, host, port, path, AND query string. No normalization, no prefix matching, no query wildcards. If you need to propagate per-request CSRF / PKCE state, carry it out-of-band (\`sessionStorage\`, first-party cookie) — never on the URL. See Phase 3.1.
