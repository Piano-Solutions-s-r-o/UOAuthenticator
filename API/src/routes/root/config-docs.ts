...
export const configJwtDocumentation = {
  ...
  optional_fields: {
    ...
    avatars: {
      default_style:
        '"tiles" | "waves" | "rings" | "mono" (optional) — the domain default style for UOA-generated fallback avatars. Absent means a stable per-user pick derived from the user id. A `?style=` query parameter on the avatar GET endpoints overrides this. Any other value rejects the config.',
    },
    session: {
